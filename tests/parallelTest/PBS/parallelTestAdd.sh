...
# Figure out number of cores to use this time - should be a multiple of ncores_per_numanode
CORE_ARRAY=( 1 2 4 6 )
NCORES=${CORE_ARRAY[${NJOB}]}

export ncores_per_task=${NCORES}
...
CMDS_PATH=$HOME/tests/$JOBNAME/PBS/cmds_add.txt
...
cat ./slim_time* >> $SAVEDIR/slim_time_n1000_add.csv
...
# 
    ...
    cd $PBS_O_WORKDIR
    qsub -v NJOBS=$NJOBS,NJOB=$NJOB ./${JOBNAME}Add.sh
else
