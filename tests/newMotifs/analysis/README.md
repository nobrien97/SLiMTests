...
## Per molecular-component ruggedness

I estimated how much changes in each molecular component separately contributed to ruggedness and holeyness. I did this by repeating the above method, but changing each molecular component one at a time with randomly sampled mutations. The same mutation was applied to each component under 10 different genetic backgrounds and for 1000 sampled mutations. 

![](../fitnessLandscape/ruggedness/R/plt_landscaperuggedness_permolcomp.png)

This figure shows the ruggedness contributions of mutations in each component. Baseline has strong effects in NAR, PAR, and FFLC1 on ruggedness, but less so in the two more holey landscapes. aZ looks like it marginally increases ruggedness in FFBH, but apart from that FFBH ruggedness remains low always. FFLI1 is most incrased by baseline expression mutations, FFLC1 by aZ mutations.

![](../fitnessLandscape/ruggedness/R/plt_landscapeholeyness_permolcomp.png)

aZ immediately stands out as producing many fitness holes in both FFLI1 and FFBH, but no so much in the other networks. bZ has a similar effect, but the NAR also has many fitness holes generated by this component. Baseline expression is also a powerful generator of fitness holes in this way.

![](../fitnessLandscape/ruggedness/R/plt_landscaperuggedness_permolcomp_prop.png)

This figure shows the proportion of total mutations per molecular component which generated a fitness hole in each model. Again aZ, bZ, and baseline standout for FFBH compared to the others. bZ in general seems to generate fitness holes, as does baseline.

Now we might expect that additive variance would be distributed differently between molecular components if some have a propensity for reducing fitness or are difficult to navigate. So we can have a look at that

## Additive variance
![](../h2/getH2/R/plt_va.png)

First I looked at the distribution of variance in fitness between models. The data is quite noisy, I will have to run some more replicates. The NAR seems to be most able to maintain additive variance, whereas the FFBH has trouble (especially with high recombination: maybe very positive epistasis being broken up?). But otherwise, the models all seem similar.


![](../h2/getH2/R/plt_va_percomp.png)

This is a lot, but it chows a clear lack of VA in aZ and bZ in the FFBH model among maladapted populations. However, in the adapted populations there is a bit of VA in those molecular components: perhaps some segregating variants that aren't producing a fitness hole? 

![](../h2/getH2/R/plt_deltaVA.png)

Change in VA between the start and end: not much difference.

![](../h2/getH2/R/plt_tree_gmatrix_full.png)

Clustering showing similarity in G matrix structure between models. Unsurprisingly the NAR/PAR are similar, as are the FFLs. Same with the FFBH being close to the FFLs, this is essentially reproducing the network diagrams.

![](../h2/getH2/R/PCASim_r_modelCombo.png)

Here we have a PCA similarity plot between comparisons of matrices - NAR/PAR vs FFLs are very different, between similar matrices they are similar. However, FFLs are pretty different to FFBH in the components used to adapt.

![](../h2/getH2/R/plt_evol.png)

The last figure shows the evolvability metrics for each model among adapted populations. The FFBH does poorly on all of these, reflecting strong constraint (a cost of complexity).