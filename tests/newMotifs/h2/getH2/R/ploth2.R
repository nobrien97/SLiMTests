...
d_h2_mrr <- read_csv("../../data/out_h2_mrr.csv", col_names = F)
d_h2_mkr <- read_csv("../../data/out_h2_mkr.csv", col_names = F)


colnames(d_h2_mrr) <- c("gen", "seed", "modelindex", "VA_w", "h2_w", "VA_aX", "VA_KZX", 
                        "VA_aY", "VA_bY", "VA_KY", "VA_aZ", "VA_bZ", "VA_KZ", "VA_KXZ", 
                        "VA_base", "VA_n", "VA_XMult", "aX_KZX", "aX_aY", "aX_bY", "aX_KY", 
                        "aX_aZ", "aX_bZ", "aX_KZ", "aX_KXZ", "aX_base", "aX_n", "aX_XMult", 
                        "KZX_aY", "KZX_bY", "KZX_KY", "KZX_aZ", "KZX_bZ", "KZX_KZ", "KZX_KXZ", 
                        "KZX_base", "KZX_n", "KZX_XMult", "aY_bY", "aY_KY", "aY_aZ", 
                        "aY_bZ", "aY_KZ", "aY_KXZ", "aY_base", "aY_n", "aY_XMult", "bY_KY", 
                        "bY_aZ", "bY_bZ", "bY_KZ", "bY_KXZ", "bY_base", "bY_n", "bY_XMult", 
                        "KY_aZ", "KY_bZ", "KY_KZ", "KY_KXZ", "KY_base", "KY_n", "KY_XMult", 
                        "aZ_bZ", "aZ_KZ", "aZ_KXZ", "aZ_base", "aZ_n", "aZ_XMult", "bZ_KZ", 
                        "bZ_KXZ", "bZ_base", "bZ_n", "bZ_XMult", "KZ_KXZ", "KZ_base", 
                        "KZ_n", "KZ_XMult", "KXZ_base", "KXZ_n", "KXZ_XMult", "base_n", 
                        "base_XMult", "n_XMult", "h2_aX", "h2_KZX", "h2_aY", "h2_bY", 
...
# Add our variables
combos <- read_delim('/mnt/c/GitHub/SLiMTests/tests/newMotifs/R/combos.csv', 
                     delim = " ", col_names = F)
names(combos) <- c("model", "rwide")
d_h2 %>% mutate(model = combos$model[.$modelindex],
                rwide = combos$rwide[.$modelindex]) -> d_h2

d_h2 |>
  dplyr::summarise(n = dplyr::n(), .by = c(gen, seed, modelindex, model, rwide, calcMode)) |>
  dplyr::filter(n > 1L)


ggplot(d_h2 %>% distinct(gen, seed, modelindex, calcMode, .keep_all = T) %>% 
         select(gen, seed, modelindex, model, rwide, h2_w, calcMode) %>%
         distinct() %>%
...
ggplot(d_h2 %>% distinct(gen, seed, modelindex, calcMode, .keep_all = T) %>% 
         select(gen, seed, modelindex, model, rwide, VA_w, calcMode) %>%
         distinct() %>%
