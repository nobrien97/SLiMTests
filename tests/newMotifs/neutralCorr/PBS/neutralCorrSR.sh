...
JOBNAME=newMotifs
SUBJOBNAME=neutralCorr
FULLJOBNAME=$JOBNAME/$SUBJOBNAME
...
SEED_FILE=$TESTDIR/R/${SUBJOBNAME}_seeds.csv
SEED_NUM=($(awk "NR==$SEED" $SEED_FILE))

# Run the model
echo "Running modelindex = $MODELINDEX, seed = $SEED...\n"
$HOME/SLiM/chp3/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d modelType=${MODEL_NUM[0]} $TESTDIR/slim/baseScript.slim
