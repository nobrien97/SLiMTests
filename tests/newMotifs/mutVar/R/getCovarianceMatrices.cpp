...
List extractCovarianceMatrices(DataFrame df) {
  ...
  for (int i = 0; i < col_names.size(); ++i) {
    ...
    else if (col_name.substr(6,1) == "c") {
      // Covariances
      size_t first_underscore = col_name.find("_");
      std::string var1 = col_name.substr(0, 2);
      std::string var2 = col_name.substr(first_underscore+1, 2);
...
//               var.c_str(), value, idx);
        ...
        size_t first_underscore = col_name.find("_", 2);
        std::string var1 = col_name.substr(0, 2);
        std::string var2 = col_name.substr(first_underscore + 1, 2);
