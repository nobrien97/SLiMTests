...
CalcNewOptimumAlongVector <- function(oldOptimum, sigma, desiredFitnessAtShift, dir) {
	# Calculates a new optimum given a given direction

	# Last value of dir is the eigenvalue, need to scale by that
	lambda = dir[length(dir)];
	dir = dir[1:(length(dir) - 1)];

	n = length(oldOptimum);

	denom = sum( (dir * dir) / (sigma * sigma) );
	D = sqrt(-2 * log(desiredFitnessAtShift)/ (lambda * denom));
	
	optNew = oldOptimum + D * sqrt(lambda) * dir;
	return(optNew);
}
