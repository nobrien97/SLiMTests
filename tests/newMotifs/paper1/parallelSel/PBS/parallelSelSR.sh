...
JOBNAME=newMotifs
FULLJOBNAME=$JOBNAME/paper1/parallelSel
...
$HOME/SLiM/chp3/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d identifier=\"${FILENAME}\" -d molTraitFix=-1 -d modelType=${MODEL_NUM[0]} -d rwide=${MODEL_NUM[1]} $TESTDIR/slim/parallelSel.slim
