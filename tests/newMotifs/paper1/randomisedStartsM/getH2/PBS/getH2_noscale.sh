...
JOBNAME=getH2
JOBNAME_PREFIX=newMotifs/paper1/randomisedStartsM
...
CMDS_PATH=$HOME/tests/$TOTALJOBNAME/PBS/cmds_noscale.txt
...
cat ./*_noscale_trait_mrr.csv >> $SAVEDIR/out_h2_noscale_trait_mrr.csv
cat ./*_noscale_trait_mkr.csv >> $SAVEDIR/out_h2_noscale_trait_mkr.csv

# Remove trait_ before they are added to molcomp h2
rm $(ls -1 | grep -E "*.trait_.*")
cat ./*_noscale_mrr.csv >> $SAVEDIR/out_h2_noscale_mrr.csv
cat ./*_noscale_mkr.csv >> $SAVEDIR/out_h2_noscale_mkr.csv
