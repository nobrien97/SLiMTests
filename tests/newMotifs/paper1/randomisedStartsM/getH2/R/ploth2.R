...
DATA_PATH <- "/mnt/c/GitHub/SLiMTests/tests/newMotifs/paper1/randomisedStartsM/getH2/R/"
...
d_h2_trait_mkr <- read_csv(paste0(DATA_PATH, "out_h2_trait_mkr.csv"), col_names = F)
d_h2_trait_mrr <- read_csv(paste0(DATA_PATH, "out_h2_trait_mrr.csv"), col_names = F)
...
ggplot(d_h2_trait %>% distinct(gen, seed, modelindex, calcMode, .keep_all = T) %>% 
...
# convert optimum to selection vector based on mean phenotype
# Opt - mean phenotype -> magnitude and direction of selection
