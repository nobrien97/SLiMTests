...
JOBNAME=newMotifs
FULLJOBNAME=$JOBNAME/extraReplicates
BASETESTDIR=$HOME/tests/$JOBNAME        # This is where the combos and slim files live
...
MODEL_FILE=$BASETESTDIR/R/combos.csv
...
$HOME/SLiM/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d molTraitFix=-1 -d modelType=${MODEL_NUM[0]} -d rwide=${MODEL_NUM[1]} $BASETESTDIR/slim/baseScript.slim
