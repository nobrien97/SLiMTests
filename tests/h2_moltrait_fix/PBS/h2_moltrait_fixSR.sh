...
# Run the model
$HOME/SLiM/slim -s $SEED_NUM -d modelindex=$MODELINDEX -d molTraitFix=${MODEL_NUM[0]} -d nloci=${MODEL_NUM[1]} -d width=${MODEL_NUM[2]} -d locisigma=${MODEL_NUM[3]} $TESTDIR/slim/$MODEL
