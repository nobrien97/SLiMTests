...
DATAPATH=/g/data/ht96/nb9894/standingVar/adjustedTau
...
if [[ "${MODEL_NUM[3]}" == "'K'" ]]
then
    $HOME/SLiM/chp2/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d inputSeed=${SEED_NUM} -d inputModel=$MODELINDEX -d molTraitFix=-1 -d molTraitProps="c(0.25, 0.25, 0.25, 0.25)"  -d modelType="'ODE'" $TESTDIR/slim/calcMutVar_adjTau.slim
else
    $HOME/SLiM/chp2/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d inputSeed=${SEED_NUM} -d inputModel=$MODELINDEX -d molTraitFix="c(2,3)" -d molTraitProps="c(0.5, 0.5, 0.0, 0.0)" -d modelType="${MODEL_NUM[3]}" $TESTDIR/slim/calcMutVar_adjTau.slim
fi
