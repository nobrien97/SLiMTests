...
# plot adapted/maladapted data separately

d_com_adapted <- readRDS("./d_comh2_prefiltered_adapted.RDS")
d_com_maladapted <- readRDS("./d_comh2_prefiltered_maladapted.RDS")
d_com_wasadapted <- readRDS("./d_comh2_prefiltered_wasadapted.RDS")

d_qg_a <- d_com_adapted %>% group_by(gen, model, nloci, sigma) %>%
  distinct(gen, seed, model, nloci, sigma, .keep_all = T) %>%
  summarise(He = mean(meanH),
            seHe = se(meanH),
            meanPheno = mean(phenomean),
...
pheno_time <- ggplot(d_qg_a %>% mutate(gen = gen - 50000), 
                     aes(gen, meanPheno, color = model)) +
  facet_grid(nloci~sigma) +
  geom_line() +
  geom_hline(yintercept = 2, linetype = "dashed") +
...
ggsave("pheno_time_adapted.png", pheno_time, width = 10, height = 7, bg = "white")

d_qg_m <- d_com_maladapted %>% group_by(gen, model, nloci, sigma) %>%
  distinct(gen, seed, model, nloci, sigma, .keep_all = T) %>%
  summarise(He = mean(meanH),
            seHe = se(meanH),
            meanPheno = mean(phenomean),
...
pheno_time <- ggplot(d_qg_m %>% mutate(gen = gen - 50000), 
                     aes(gen, meanPheno, color = model)) +
  facet_grid(nloci~sigma) +
  geom_line() +
  geom_hline(yintercept = 2, linetype = "dashed") +
...
ggsave("pheno_time_maladapted.png", pheno_time, width = 10, height = 7, bg = "white")


d_qg_w <- d_com_wasadapted %>% group_by(gen, model, nloci, sigma) %>%
  distinct(gen, seed, model, nloci, sigma, .keep_all = T) %>%
  summarise(He = mean(meanH),
            seHe = se(meanH),
            meanPheno = mean(phenomean),
...
pheno_time <- ggplot(d_qg_w %>% mutate(gen = gen - 50000), 
                     aes(gen, meanPheno, color = model)) +
  facet_grid(nloci~sigma) +
  geom_line() +
  geom_hline(yintercept = 2, linetype = "dashed") +
...
ggsave("pheno_time_wasadapted.png", pheno_time, width = 10, height = 7, bg = "white")
...
saveRDS(d_freqs, "d_freqs_adapted.RDS")
...
d_com_adapted %>%
  mutate(molTrait = recode_factor(mutType, 
                                  `1`="Neutral", `2`="Del", `3`="$$\\alpha_Z$$", `4`="$$\\beta_Z$$", 
                                  `5`="$$K_Z$$", `6`="$$K_{XZ}$$")) -> d_com_adapted

# Lets see the total distribution to see how big these values get
d_com_adapted %>% filter(model == "NAR", gen == 51950, Freq == 1) %>%
  ggplot(aes(x = log(value), fill = molTrait)) + 
  geom_histogram(bins = 100)

d_com_adapted %>% filter(model == "Additive", gen == 51950) %>%
  ggplot(aes(x = value)) +
  geom_histogram(bins = 50)


# Split the data set into a list of groups - each is a collection of allelic effects
MIN_VAL <- min(log(d_com_adapted[d_com_adapted$model == "NAR",]$value))
MAX_VAL <- max(log(d_com_adapted[d_com_adapted$model == "NAR",]$value))

d_com_adapted %>% filter(model == "NAR") %>%
  mutate(value = log(value)) %>%
  mutate(val_bin = cut(value,
                        breaks = seq(from = MIN_VAL, to = MAX_VAL, by = 0.05))) %>% # categorise freq
  group_by(gen, seed, nloci, sigma) %>%
...
d_values_NAR <- readRDS("d_values_NAR_adapted.RDS")
...
d_values_NAR %>%
  ...
  # Save data for stats
saveRDS(d_values_NAR, "d_values_NAR_adapted.RDS")
...
MIN_VAL <- min(d_com_adapted[d_com_adapted$model == "Additive",]$value)
MAX_VAL <- max(d_com_adapted[d_com_adapted$model == "Additive",]$value)

d_com_adapted %>% filter(model == "Additive") %>%
  mutate(val_bin = cut(value,
                       breaks = seq(from = MIN_VAL, to = MAX_VAL, by = 0.05))) %>% # categorise freq
  group_by(gen, seed, nloci, sigma) %>%
  mutate(binCount = n()) %>%
...
ggsave("val_end_adapted.png", plt_val, width = 7, height = 10, bg = "white")
...
saveRDS(d_meanfreq_NAR, "meanfreq_nar_test.RDS")
...
ggsave("meanVal_adapted.png", plt_meanVal, width = 10, height = 10, bg = "white")
...
ggsave("meanFreq_adapted.png", plt_meanFreq, width = 7, height = 10, bg = "white")



# Heritability and predictions
###################################################
d_qg_sum <- d_com_adapted %>% group_by(gen, model, nloci, sigma) %>%
  drop_na(AIC) %>%
  distinct(gen, seed, model, nloci, sigma, .keep_all = T) %>%
  mutate(gen = gen - 50000) %>%
  summarise(He = mean(meanH),
