...
path <- "/mnt/d/SLiMTests/tests/newTestCross/getH2_newTestCross/data/"

# Read in data
d_qg <- read_csv(paste0(path, "slim_qg.csv"), col_names = F)
d_h2 <- read_csv(paste0(path, "out_h2.csv"), col_names = F)
...
d_combined_full <- full_join(d_combined, d_h2, by = c("gen", "seed", "modelindex", "fixedEffect", "nloci", "sigma"))
