...
combos <- read_delim("~/tests/newTestCross/R/combos.csv", delim = " ", col_names = F)
...
d_combined_after %>% filter(fixedEffect == -1) %>% mutate(h2A = cut(H2.A.Estimate, breaks = c(0, 0.25, 0.5, 0.75, 1), include.lowest = T),
