...
d_new <- readRDS("/g/data/ht96/nb9894/equalK_sweep/checkpoint/d_indPheno_adapted.RDS")


# Get appropriate subset - 42 timepoints, 10 individuals per timepoint
d_new <- d_new[((run-1)*420+1):(run*420),]
...
for(i in 1:nrow(d_new)) {
  ...
  res <- ode(state, times, nar, params, "rk4") %>%
    ...
    mutate(gen = d_new$gen[i], seed = d_new$seed[i], modelindex = d_new$modelindex[i],
           ...
           ind = d_new$ind[i],
           phenotype = d_new$phenotype[i],
           X = as.integer((time > Xstart & time <= Xstop))) %>%
    select(gen, seed, modelindex, nloci, sigma, ind, phenotype, time, X, Z)
